    ...
    <div class="text-c">
        @forelse ($search['inputs'] as $input)
            <div class="form-group">
                @if($input['type'] == 'text')
                    <input type="text" name="{{$input['name']}}" id="{{$input['name']}}" style="width:250px"
                ...
                @elseif($input['type'] == 'date')
                    日期范围：
                    <input type="text"
                           onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'{{$input['name']}}Max\')||\'%y-%M-%d\'}',dateFmt:'yyyy-MM-dd 00:00:00'})"
                           id="{{$input['name']}}Min" name="{{$input['name']}}Min"
                           value="{{old($input['name'].'Min')}}"
                           class="input-text Wdate" style="width:120px;">
                    -
                    <input type="text"
                           onfocus="WdatePicker({minDate:'#F{$dp.$D(\'{{$input['name']}}Min\')}',maxDate:'%y-%M-%d',dateFmt:'yyyy-MM-dd 23:59:59'})"
